...
import {
  ...
  getGetRewardsQueryKey, getListBookingsQueryKey, useCancelBooking, useCompleteBooking, useCreateBooking,
...
function LoginPage() {
  ...
  return <main className="lh-page"><div className="lh-container"><div className="lh-form-card"><div className="lh-kicker lh-mono">Welcome back</div><h1 className="lh-display">Come on in.</h1><p>Your saved stays and next good trip are waiting.</p>{error && <div className="lh-alert" data-testid="status-login-error">{error}</div>}<form onSubmit={submit}><div className="lh-form-field"><label htmlFor="login-email">EMAIL</label><input className="lh-input" id="login-email" type="email" autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)} required data-testid="input-login-email" /></div><div className="lh-form-field"><label htmlFor="login-password">PASSWORD</label><input className="lh-input" id="login-password" type="password" autoComplete="current-password" minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} required data-testid="input-login-password" /></div><button className="lh-btn lh-btn-primary lh-btn-block" type="submit" disabled={login.isPending} data-testid="button-login">{login.isPending ? 'Checking…' : 'Sign in'} <ArrowRight size={15} /></button></form><div className="lh-form-foot"><span>New to likehome?</span><Link href="/signup" data-testid="link-signup">Create an account</Link></div></div></div></main>;
}
